function editor_antenas()
% editor_antenas — EDITOR INTERACTIVO: mueve y orienta cada antena a tu gusto.
%   · Menú para elegir la antena (H1..H5, C1..C7)
%   · Sliders: X, Y, altura Z y ORIENTACIÓN (0-360°)
%   · El patrón se redibuja en vivo (usa la caché, instantáneo)
%   · Botón "Trazar rayos" = rayos SBR reales desde la antena seleccionada
%   · Botón "Guardar posiciones" -> antenas_editadas.csv
TRB = 'C:\Users\Adrian Lopez\Documents\tesis_proyecto\fase4\trabajo\';
C = load([TRB 'escena_cache.mat']);               % pHawkBi pCard az el ...
pHawkBi = C.pHawkBi; pCard = C.pCard; az = C.az; el = C.el;
...
hPat = gobjects(NA,1); hMk = gobjects(NA,1); hTx = gobjects(NA,1); hRays = gobjects(0);
...
uicontrol('Style','popupmenu','Units','normalized','Position',[px 0.87 pw 0.04], ...
...
uicontrol('Style','pushbutton','Units','normalized','Position',[px 0.45 pw 0.05], ...
    'String','Trazar rayos desde la antena','FontSize',10,'Callback',@onRays);
...
uicontrol('Style','text','Units','normalized','Position',[px 0.05 pw 0.24], ...
    ...
    '4. "Trazar rayos" = rayos SBR reales a 12/24/38 m en la direccion del lobulo.\n' ...
...
% ================= funciones anidadas (comparten variables) =================
    function h = mkslider(y, txt, mn, mx, val, cb)
        uicontrol('Style','text','Units','normalized','Position',[px y+0.035 pw 0.03], ...
            'String',txt,'FontSize',10,'ForegroundColor','w', ...
            'BackgroundColor',[0.05 0.05 0.07],'HorizontalAlignment','left');
        h = uicontrol('Style','slider','Units','normalized','Position',[px y pw 0.035], ...
            'Min',mn,'Max',mx,'Value',val,'Callback',cb);
    end

    function D = patronLobulo(pat, cx, cy, cz, esc, ang, girar)
    ...
    function redibujar()
        for q = 1:NA
            ...
            if q==S.sel, mkS = 15; col2=[1 1 0.3]; else, mkS = 8; col2=col; end
    ...
    function onRays(~,~)
        ...
        hRays = gobjects(0);
        q = S.sel;
        set(fig,'Name','Editor de antenas — calculando rayos SBR...'); drawnow;
        ...
        for d = [12 24 38]
            ...
            for r2 = 1:numel(rr{1})
                ...
                for it = 1:numel(R.Interactions), pts=[pts; R.Interactions(it).Location(:)']; end %#ok<AGROW>
                pts = [pts; Pl(1) Pl(2) 1.2]; %#ok<AGROW>
                f2 = min(max((R.PathLoss-60)/60,0),1); c = cmap(max(1,257-round(1+f2*255)),:);
                hRays(end+1) = plot3(ax, pts(:,1),pts(:,2),pts(:,3),'-','Color',[c 0.5],'LineWidth',1); %#ok<AGROW>
            end
            nTot = nTot + numel(rr{1});
        end
        set(fig,'Name',sprintf('Editor de antenas — %d rayos desde %s', nTot, IDS{q}));
    end
end
